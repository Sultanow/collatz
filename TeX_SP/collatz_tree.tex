...
\[
v_{3+1}=\frac{3^3\cdot(7+1)-2^3}{2^3}=107
\]

\par\noindent
We use formula~\ref{eq:engel} to verify theorem~\ref{eq:max_alpha} by proving that dividing $\hat\alpha$ times by two will lead to a result $v_{n+2}<2$ for every worst-case sequence.\footnote{The case is hypothetical. Not every worst-case sequence actually ends with $v_{n+2}=1$. We consequently have to define the condition $v_{n+2}<2$ in order to cover all possible sequences.} For this purpose, we extend formula~\ref{eq:engel} by an additional step and divide by $\hat\alpha-n$ in the final iteration:
\[
