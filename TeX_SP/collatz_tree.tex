...
\end{array}
\end{equation}
A validation with the above mentioned empirical data set confirms the limits of $\beta$ formulated by theorem~\ref{eq:boundary_beta}.\footnote{Source: Own empirical analysis, see appendix "\nameref{appx:data_set}" for details.} The following diagram shows the maximum of $\beta$ for different values of $k$ in the data set. It as well depicts the corresponding starting number $v_1$ that led to this maximum.

\begin{figure}
	\includegraphics[trim=0 395 50 0, clip, width=1.00\textwidth]{figures/beta_dashboard.pdf}
	\caption{A little explanation what this chart shows}
	\label{fig:1}
\end{figure}

As we can see in the visualisation above, the maximum for $k=1$ equals $2$. For other $k$ the limit is even lower. The maximum for $k=3$, for example, equals $1.\overline{3}=\frac{4}{3}$. The diagram further reveals that the maximum $\beta$ for every $k$ is reached for the starting number $v_1=1$. A proof for this hypothesis will be provided in a future paper.
