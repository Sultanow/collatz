...
Based on theorems ~\ref{eq:reachability_k} and ~\ref{eq:cycle_alpha} we can straightforwardly derive the parameter $\bar\beta$ that is required for a cycle of a specific length. To this end, we examine the situation in which theorem~\ref{eq:reachability_k} leads to a cycle. Consequently, we set $v_{n+1}=v_1$ and $\alpha=\bar\alpha$:
\begin{equation}
\label{eq:cycle_beta}
\def\arraystretch{1.2}\begin{array}{l}
v_1=v_1 \cdot 3^n \cdot \beta \cdot 2^{-\bar\alpha}\\
1=3^n \cdot \beta \cdot 2^{-\bar\alpha}\\
\beta =3^n \cdot 2^{-\lfloor n\cdot\log_2k\rfloor-1}\\
\end{array}
\end{equation}

By proving that the maximum of $\beta$ for $k=3$ in fact equals $\frac{4}{3}$, as proposed in section "\nameref{sec:analysing_beta}", one could disprove all cycles that require a $\bar\beta$ above this limit.
