...
\end{array}
\end{equation}
The limits formulated by formula~\ref{eq:boundary_beta} are confirmed through a validation with our empirical data set. Figure~\ref{fig:1} shows the maximum $\beta$ for different values of $k$ in the sample. The diagram as well depicts the corresponding starting number $v_1$, which leads to this maximum.
\begin{figure}[H]
\begin{center}
\includegraphics[trim=40 45 60 40, clip, width=0.79\textwidth]{figures/beta_dashboard.pdf}
