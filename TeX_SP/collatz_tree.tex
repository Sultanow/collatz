...
\par\noindent
The above term represents the (hypothetical) case in which a sequence rises to its maximum for a specific starting value $v_1$. For the Collatz conjecture this is the worst-case scenario because the equation never ends with one due to the exponential increase. Let us consider the example $v_1=7$ and $n=1$. Applying equation~\ref{eq:engel} yields:
\[
...
\par\noindent
We use formula~\ref{eq:engel} to verify theorem~\ref{eq:max_alpha} by proving that dividing $\hat\alpha$ times by two will lead to a result $v_{n+2}<2$ for every worst-case sequence.\footnote{The case is hypothetical. Not every worst-case sequence actually ends with $v_{n+2}=1$. Consequently we have to define the condition $v_{n+2}<2$ in order to cover all possible sequences.} For this purpose, we extend formula~\ref{eq:engel} by an additional step and divide by $\hat\alpha-n$ in the final iteration:
\[
...
2^{\hat\alpha+1}-2^n+3*2^n~>~3^{n+1}\cdot(v_1+1)\\
