...
Based on theorems ~\ref{eq:reachability_k} and ~\ref{eq:cycle_alpha} we can derive the parameter $\bar\beta$ that is needed for a cycle of a certain length. For this purpose we investigate the scenario in which theorem~\ref{eq:reachability_k} leads to a cycle by setting $v_{n+1}=v_1$ and $\alpha=\bar\alpha$:
\begin{equation}
\label{eq:cycle_beta}
\def\arraystretch{1.2}\begin{array}{l}
v_1=3^n \cdot v_1\cdot \bar\beta \cdot 2^{-\bar\alpha}\\
1=3^n \cdot \bar\beta \cdot 2^{-\bar\alpha}\\
\frac{1}{\bar\beta} =3^n \cdot 2^{-\lfloor n\cdot\log_2k\rfloor-1}\\
