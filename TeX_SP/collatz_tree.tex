...
\end{equation}
The above term proves theorem~\ref{eq:max_alpha} for the case $k=3$. An alternative verification of theorem~\ref{eq:max_alpha} can be found in the appendix. A closer look makes clear that it is not only valid for $k=3$, but for all $k$. In conclusion, we can define the following boundaries for the number of divisions by two in a Collatz sequence:
\begin{equation}
