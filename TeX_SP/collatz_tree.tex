...
We verify theorem~\ref{eq:max_alpha} for $k=3$ with the following equation. Formula~\ref{eq:engel} builds on the so-called Engel expansion and calculates the odd number $v_{n+1}$ for a Collatz sequence in which we divide by two only once per iteration:\footnote{See Laarhoven \cite[p.~11]{Ref_Laarhoven_2009}}
