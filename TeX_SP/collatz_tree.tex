...
\end{array}
\end{equation}
A validation with the above mentioned empirical data set confirms the limits formulated by theorem~\ref{eq:boundary_beta}.\footnote{Source: Own empirical analysis, see appendix "\nameref{appx:data_set}" for details.} The following diagram shows the maximum $\beta$ for different values of $k$ in the data set. It as well depicts the corresponding starting number $v_1$ which leads to this maximum.
...
Proving that the maximum $\beta$ for $k=3$ in fact equals $\frac{4}{3}$, as proposed in section "\nameref{sec:analysing_beta}", would enable us to rule out all cycles that require a $\bar\beta$ above this limit.
