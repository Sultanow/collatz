...
class AberkaneTree:
    ...
    def mark(self, number: int) -> Tuple[int, int]:
...
def generateAberkaneTree(height: int, count: int, start: int = 1) -> None:
    ...
    if node is not None:
        ...
        print(
            "Warning: Could not add number {0} to start number".format(start))

    for r in range(count):
        calculateNextNode(tree)

    return tree


def calculateNextNode(tree: AberkaneTree) -> None:
    for number in range(1, 2**tree.num_rows, 2):
        ...
        if node.state == NodeState.ORANGE:
            connections = []
            connection = tree.mark(V(number))
            if connection is not None:
                connections.append(connection)

            if ruleTwo(number):
                connection = tree.mark(S(number))
                if connection is not None:
                    connections.append(connection)

            if isTypeA(number):
                connection = tree.mark(Ra(number))
                if connection is not None:
                    connections.append(connection)

            if isTypeC(number):
                connection = tree.mark(Rc(number))
                if connection is not None:
                    connections.append(connection)
...
def getPos(tree: AberkaneTree, number: int) -> tuple[float, float]:
    size = 1
    row = Node.getRow(number)
...
def generatePlot(tree: Tree) -> None:
    ...
    for row in tree.rows:
        for node in row:
            if node.number == 1:
                pos[node.number] = (getPos(tree, 3)[0], 0)
            else:
...
def generateTree(count, name, height=6):
    tree = generateAberkaneTree(height, 0, 1)
...
generateTree(0, 'tree_0.svg')
generateTree(1, 'tree_1.svg')
generateTree(4, 'tree_4.svg')
generateTree(5, 'tree_5.svg')
generateTree((2**10)//2, 'tree_height_10.svg', 10)
