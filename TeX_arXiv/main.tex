...
\item choose a smallest possible $v_1$,
\item choose an $\alpha$ as large as possible and
\item keep $n$ to a minimum (which is contrary to maximizing $\alpha$)
\end{itemize}
...
\par\medskip\noindent
{\bfseries\boldmath Maximizing $\alpha$ at lowest $n$:} Maximizing $\alpha$ while keeping $n$ as minimal as possible means that we must choose a preferably short Collatz sequence, but one in which as many divisions by two as possible occurred. The value $n$ depends directly on $\alpha$.
...
\par\medskip
Maximizing the product of reciprocated Collatz sequence members $\prod_{i=1}^{n}(1+\nicefrac{1}{kv_i})$ requires us to maximize the equation~\ref{eq:prod_engel_more_divisions} or \ref{eq:prod_engel_more_divisions_v2}. This maximization is archieved by choosing $v_1$ as small as possible (which we already know). Additionally we need to select a largest possible $\alpha_1$ or a lowest possible $v_2$ (which is clear as well). Lastly, this maximization also requires the choice of a value $n$ as large as possible. Why $n$ large as possible? As mentioned before, $n$ is directly dependent on $\alpha$, and specifically $\alpha=\alpha_1+n-1$. In other words, maximizing $\alpha$ is equivalent to maximizing $n$ (turning one of both setting screws is enough). To find the largest possible product, we iterate the variables $v_1$ and $n$ independently through integers $1,2,\ldots$ and calculate the product for each combination of the variable assignment using equation~\ref{eq:prod_engel_more_divisions_v2}.
...
\par\medskip
One may say that there exist some Collatz sequences, which are not an Engel expansions but their product might be larger than the product resulting from the corresponding Engel expansion, for example:
...
{\renewcommand{\arraystretch}{1.8}
	\begin{table}[H]
		...
		\end{tabular}
		\caption{Formulas that calculate the Engel expansion's product for $k=1,3,5,7$}
		\label{table:comparison_products}
\end{table}}
