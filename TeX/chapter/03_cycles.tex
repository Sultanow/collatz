...
In order to prove the upper limit of $\bar\alpha$ for $k=3$, we have to show that condition~\ref{eq:condition_max} is true. To improve readability, we denote the factor $(1+\frac{1}{3v_i})$ with $\beta_i$:
\begin{equation*}
...
\begin{example}
	Setting $v_1=13$ and $n=1$ leads to:
	\[
...
v_{n+2}&=\frac{3^{n+1}v_1\prod_{i=1}^{n+1}\beta_i}{\left(\prod_{i=1}^{n}2^{\alpha_i}\right)2^{\alpha_{n+1}}}\\
v_{n+2}&=\frac{3^{n+1}\cdot v_1\cdot2}{3^{n+1}v_1\left(2-\prod_{i=1}^{n}\beta_i\right)\cdot2^{\alpha_{i+1}}}\\
